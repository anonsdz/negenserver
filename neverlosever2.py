...
async def attack_method(update, context, methods_data, vip_users): 
    ...
    if chat_id != GROUP_ID: return await update.message.reply_text("Bạn không có quyền sử dụng methods VIP . Buy VIP Liên hệ [@neverlose102 👑]")
    if user_id in user_processes and user_processes[user_id].returncode is None: return await update.message.reply_text("Đang có cuộc ATTACKER đang chạy.")
    ...
    if method['visibility'] == 'VIP' and user_id != ADMIN_ID and user_id not in vip_users: return await update.message.reply_text("Bạn không có quyền sử dụng methods VIP . Buy VIP Liên hệ [@neverlose102👑]")
...
async def execute_attack(command, update, method_name, start_time, attack_time):
    ...
    await update.message.reply_text(f"ATTACKER đã hoàn tất! Thời gian: {elapsed_time}s.\n\nChi tiết:\n<pre>{safe_attack_info_text}</pre>", parse_mode='HTML')
